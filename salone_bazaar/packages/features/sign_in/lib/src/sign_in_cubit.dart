...
final class SignInCubit extends Cubit<SignInState> {
  ...
  Future<void> onSubmit() async {
    ...
    final newState = state.copyWith(
      email: email,
      password: password,
      emailAndPasswordSubmissionStatus: isFormValid ? EmailAndPasswordSubmissionStatus.inProgress : null,
    );
    ...
    if (isFormValid) {
      try {
        ...
        emit(state.copyWith(emailAndPasswordSubmissionStatus: EmailAndPasswordSubmissionStatus.success));
      } catch (e) {
        emit(
          state.copyWith(
    ...
    emit(state.copyWith(googleSignInSubmissionStatusStatus:  GoogleSignInSubmissionStatusStatus.inProgress));

    try {
      final  googleUser = await api.auth.signInWithGoogle();
      if (googleUser == null) {
        emit(state.copyWith(googleSignInSubmissionStatusStatus:  GoogleSignInSubmissionStatusStatus.cancelled));
        return;
      }

      
      emit(state.copyWith(googleSignInSubmissionStatusStatus:  GoogleSignInSubmissionStatusStatus.success));
    } catch (error) {
      emit(state.copyWith(googleSignInSubmissionStatusStatus:  GoogleSignInSubmissionStatusStatus.error));
    }
  }
}
